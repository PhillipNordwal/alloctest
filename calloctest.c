...
/* calloc_test returns 0 if size was not able to be calloc'ed
 * @param size [in] number of char's to calloc
 * @return 0 if size wasn't able to be calloc'ed, something else otherwise
 */
unsigned short calloc_test(size_t size) {
	unsigned short retVal = 0;
	char * pChar = 0;
	static size_t sizeChar = sizeof(char);
	pChar = calloc(size, sizeChar);
	retVal = pChar != 0;
	free(pChar);
	return retVal;
}
